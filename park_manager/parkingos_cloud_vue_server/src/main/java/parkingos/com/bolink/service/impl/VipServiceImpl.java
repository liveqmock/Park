...
public class VipServiceImpl implements VipService {
    ...
    public JSONObject createVip(HttpServletRequest req) throws Exception{
        ...
        //ist 4g查询车牌是否绑定月卡
        String car_number = RequestUtil.processParams(req, "car_number");
        boolean isOnly = checkCarNumberIsOnly( car_number );
        //该车牌已经绑定月卡
        if(!isOnly){
            return result;
        }
...
//        logger.error(">>>>>>>>>>>>>>>>"+carTypeId);
    ...
    public JSONObject editCarNum(Long id, String carNumber, Long comid) {
        ...
        //IST 4G 月租车添加唯一判断 查看是否唯一
        boolean flag = checkCarNumberIsOnly(carNumber);
        if(!flag){
            return result;
        }
...
//                    mongoDbUtils.saveLogs( request,0, 3, "修改了车牌："+carNumber);
    ...
    private boolean checkCarNumberIsOnly(String carNumber) {
        CarowerProduct carowerProduct = new CarowerProduct();
